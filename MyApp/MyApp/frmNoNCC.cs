...
namespace MyApp
{
    public partial class frmNoNCC : Form
    {
        ...
        private void frmNoNCC_Load(object sender, EventArgs e)
        {
            // Bước 1
            ...
            // Bước 2: lấy dl về
            string sQuerry = "select * from ViewNoNCC";
            SqlDataAdapter adapter = new SqlDataAdapter(sQuerry, con);

            DataSet ds = new DataSet();

            adapter.Fill(ds, "ViewNoNCC");

            dataGridViewNoKH.DataSource = ds.Tables["ViewNoNCC"];
        ...
        private void LoadComboBoxDH()
        {
            using (SqlConnection con = new SqlConnection(sCon))
            {
                try
                {
                    ...
                    string sQuerySP = "SELECT maDHN, SDT, ngayTao FROM donHangNhap";
                    SqlDataAdapter adapterSP = new SqlDataAdapter(sQuerySP, con);
                    DataTable dtSP = new DataTable();
                    adapterSP.Fill(dtSP);
                    cboMaDH.DataSource = dtSP;
                    cboMaDH.DisplayMember = "maDHN";
                    cboMaDH.ValueMember = "maDHN";
                    cboMaDH.Tag = dtSP;
                }
                catch (Exception ex)
                {
        ...
        private void cboMaDH_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataRow[] selectedRows = dt.Select($"maDHN = '{cboMaDH.SelectedValue}'");
        ...
        private void btnThemNo_Click(object sender, EventArgs e)
        {
            ...
            string sQuery = "exec kiemTraVaCapNhatNoPhaiTraNCC @maDHN, @tienNo, @ngayNoTien, @ret output";
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Thêm Nợ NCC thành công!");

                    string sQueryLastDHB = "SELECT TOP 1 maDHN FROM noPhaiTraNCC ORDER BY maNPTNCC DESC";
                    ...
                    string sQueryNoNCC = "SELECT * from ViewNoNCC";
                    SqlCommand cmdNoNCC = new SqlCommand(sQueryNoNCC, con);

                    SqlDataAdapter adapter = new SqlDataAdapter(cmdNoNCC);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            string maDHN = cboMaDH.Text; // Lấy maNPTKH từ đâu đó, ví dụ từ DataGridView
            ...
            string sQueryUpdate = "UPDATE noPhaiTraNCC SET trangThaiTraTien = @trangThaiTraTien, ngayTraTien = @ngayTraTien WHERE maDHN = @maDHN";
            ...
            using (SqlConnection con = new SqlConnection(sCon))
            {
                try
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        string sQueryNoNCC = "SELECT * from ViewNoNCC";
                        SqlDataAdapter adapter = new SqlDataAdapter(sQueryNoNCC, con);
                        DataTable dt = new DataTable();
                        adapter.Fill(dt);
        ...
        private void dataGridViewNoKH_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        {
            cboMaDH.Text = dataGridViewNoKH.Rows[e.RowIndex].Cells["maDHN"].Value.ToString();
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (ret == DialogResult.OK)
            {
                ...
                string sQuery = "delete noPhaiTraNCC where maDHN = @maDHN";
                SqlCommand cmd = new SqlCommand(sQuery, con);
                cmd.Parameters.AddWithValue("@maDHN", sMaDHN);
